...
export const CACHE_KEYS = {
   ...
   */
  team: (teamId: string) => 
    `${LINEAR_PREFIX}:team:${teamId}` as const,
  ...
  issue: (issueId: string) => 
    `${LINEAR_PREFIX}:issue:${issueId}` as const,
  ...
  userTeams: (userId: string) => 
    `user:${userId}:teams` as const,
  
  userCustomers: (userId: string) => 
    `user:${userId}:customers` as const,
  
  /**
   * Admin/System cache keys
   */
  allTeams: () => 
    `${LINEAR_PREFIX}:teams:all` as const,
  
  allCustomers: () => 
    `customers:all` as const,
  ...
  patterns: {
    ...
    allTeams: () => 
      `${LINEAR_PREFIX}:team:*` as const,
    
    allIssues: () => 
      `${LINEAR_PREFIX}:issue:*` as const,
    
    allLinear: () => 
      `${LINEAR_PREFIX}:*` as const,
  },
} as const;
